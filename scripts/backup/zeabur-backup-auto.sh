...
# ============================================
# Zeabur 自動備份腳本（使用環境變數檔案）
# ============================================
# 使用方式：
# 1. 建立 zeabur.env 檔案並填入 DATABASE_URL
# 2. 執行 ./zeabur-backup-auto.sh
# ============================================

set -e

# 使用 PostgreSQL 18 的 pg_dump
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo ""
    echo "請建立 $ENV_FILE 並加入以下內容："
    echo ""
    echo "DATABASE_URL=postgresql://user:password@host:port/database"
    echo ""
    echo "提示：從 Zeabur PostgreSQL 服務頁面複製 DATABASE_URL"
    exit 1
fi
...
# 執行備份
echo "📦 正在備份資料庫..."
if pg_dump "$DATABASE_URL" --no-sync --no-owner --no-acl > "$BACKUP_PATH"; then
